# added the parenthesis to the print statements, and fixed the indentation because there was a mix between spaces and tabs. 
...
parser.add_argument(
    "--type", help="set the type i.e. html, asp, php", dest='type')
parser.add_argument("--fast", help="uses multithreading",
...
# removes / from url so we can have example.com and not example.com/
target = target.replace('/', '')
target = 'http://' + target  # adds http:// before url so we have a perfect URL now
if args.prefix != None:
    target = target + args.prefix
try:
    # Requests to example.com/robots.txt
...
def get_paths(type):
    try:
        # opens paths.txt and grabs links according to the type arguemnt
        with open('./modules/Breacher/paths.txt', 'r') as wordlist:
...
if args.fast == True:  # if the user has supplied --fast argument
    ...
    # tells the link grabber to grab links according to user input like php, html, asp
    get_paths(type)
    ...
    def part2():
        ...
        scan(links)  # calls the scanner
    # Calls the part1 function via a thread
    t1 = threading.Thread(target=part1)
    # Calls the part2 function via a thread
